...
--[[
local genserialid = function ()
	local id = "";
	for i=1,6 do
		id = id .. string.format("%04X", math.random(0,65535));
	end
	return id;
end
...
local saves3 = "";
local saves4 = "";
for k,v in ipairs(splitstring(srcs," \
	")) do
	local serialid = genserialid();
	local s = "\t\t" .. serialid .. " = {isa = PBXFileReference; fileEncoding = 4; lastKnownFileType = sourcecode.c.c; name = "
	.. getfilename(v) .. ";path = " .. v  .."; sourceTree = SOURCE_ROOT;};";
	saves = saves .. s .. "\r";

	saves2 = saves2 .. "\t\t\t\t" .. serialid .. ",\r";

	local serialid2 = genserialid();
	saves3 = saves3 .. string.format("\t\t%s = {isa = PBXBuildFile; fileRef = %s ; };\r", serialid2, serialid);
	saves4 = saves4 .. "\t\t\t\t" .. serialid2 .. ",\r";
end

local f = io.open("temposfile.txt","w");
	f:write(saves);
	f:write("\r\n\r");
	f:write(saves2);
...
return
{
	...
	base = function(proj)
			...
			proj:AddIncludePath("./;../;../include;./asn1;./evp;../include/openssl;./modes");
	...
	targets =
	{
		...
		linux = function(proj)
			...
			proj:AddDefine("_DEFAULT_SOURCE=1");
			proj:AddFlag("-fPIC");
			proj:SetOutputPath("../../../Public/3rd/lib/linux/libcrypto.a");
		end,
		...
		android_arm = function (proj )
			...
			proj:SetOutputPath("../../../Public/3rd/lib/android/arm/libcrypto.a");
		end,
		android_x86 = function (proj)
			...
			--proj.mid_path = ("./x86");
			proj:SetOutputPath("../../../Public/3rd/lib/android/x86/libcrypto.a");
		end
	},
}